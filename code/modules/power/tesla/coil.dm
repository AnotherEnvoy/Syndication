...
/obj/machinery/power/tesla_coil/zap_act(power, zap_flags, shocked_targets)
	if(anchored && !panel_open)
		...
		if(istype(linked_techweb))
			linked_techweb.modify_points( min(power_produced, 1)) // x4 coils = ~240/m point bonus for R&D
...
/obj/machinery/power/tesla_coil/research/zap_act(power, zap_flags, shocked_targets)
	if(anchored && !panel_open)
		...
		if(istype(linked_techweb))
			linked_techweb.modify_points( min(power_produced, 3)) // x4 coils with a pulse per second or so = ~720/m point bonus for R&D
		addtimer(CALLBACK(src, .proc/reset_shocked), 10)
		zap_buckle_check(power)
		playsound(src.loc, 'sound/magic/lightningshock.ogg', 100, TRUE, extrarange = 5)
		return power_produced
