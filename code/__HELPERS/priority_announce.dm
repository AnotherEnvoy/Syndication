/proc/priority_announce(text, title = "", sound, type , sender_override, has_important_message)
	...
	if(type == "Priority")
		...
		if(!sender_override)
			if(title == "")
				GLOB.news_network.SubmitArticle(text, "Syndicate Command Update", "Station Announcements", null)
			else
				GLOB.news_network.SubmitArticle(title + "<br><br>" + text, "Syndicate Command", "Station Announcements", null)
